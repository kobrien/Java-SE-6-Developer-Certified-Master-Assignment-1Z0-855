...
public class LockManager {
     ...
     * locked record and it's corresponding client is kept in the map variable
     * lockedRecords. If the specified record is already locked the current
	...
	// lock.lock();
	// try {
	// Check if the record is already unlocked, if so then return
	if (lockedRecords.containsKey(recNo)) {
	    // Check that the locked record belongs to this client thread
